...
FEMPUTER_LIBMESH_COMMON_CONFIG_OPTIONS="--enable-everything --with-metis=PETSc --with-cppunit-prefix=${CPPUNIT_DIR:?undefined} --with-vtk-lib=${VTK_LIB:?undefined} --with-vtk-include=${VTK_INCLUDE:?undefined} --with-methods=${METHODS:?undefined} --prefix=${FEMPUTER_LIBMESH_INSTALL_DIR:?undefined}"
